...
import {
  ...
  fetchEtapaInfo, fetchFechaData, inscribirse, desinscribir,
} from "./api.js";
...
function Avatar({name,rank,size=36,border=false}) {
  return (
    <div style={{width:size,height:size,borderRadius:"50%",flexShrink:0,
      ...
      fontSize:size*0.28,fontWeight:900,color:"white",
...
export default function App() {
  ...
  const [selFecha,    setSelFecha]   = useState(null);   // null=total, 1/2/3...=fecha específica
  const [fechaCache,  setFechaCache] = useState({});     // cache por número de fecha
  ...
  // Cargar datos de una fecha específica (con cache)
  const loadFecha = useCallback(async(n)=>{
    if(fechaCache[n]) return; // ya cargada
    setFechaLoad(true); setFechaErr(null);
    try {
      const d = await fetchFechaData(n);
      setFechaCache(prev=>({...prev,[n]:d}));
    } catch(e){ setFechaErr(e.message); }
    finally{ setFechaLoad(false); }
  },[fechaCache]);

  const handleSelFecha = (n) => {
    setSelFecha(n);
    if(n!==null) loadFecha(n);
  };
  ...
  const handleToggleInsc=async()=>{
    if(!myName)return; setInscLoad(true);
    try{
          ...
          <div>
            ...
            <div style={{padding:"12px 14px 0",overflowX:"auto"}}>
              <div style={{display:"flex",gap:6,minWidth:"max-content",paddingBottom:2}}>
                {/* Tab TOTAL */}
                <button onClick={()=>handleSelFecha(null)}
                  style={{padding:"6px 14px",borderRadius:100,border:"none",cursor:"pointer",fontSize:10,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",transition:"all 0.2s",
                    background:selFecha===null?"linear-gradient(135deg,#e84fa0,#b06fc4)":"rgba(255,255,255,0.07)",
                    color:selFecha===null?"white":"rgba(255,255,255,0.45)",
                    boxShadow:selFecha===null?"0 2px 12px rgba(232,79,160,0.4)":"none"}}>
                  🏆 Total
                </button>
                {/* Tabs por fecha */}
                {Array.from({length:fechaActual},(_,i)=>i+1).map(n=>(
                  <button key={n} onClick={()=>handleSelFecha(n)}
                    style={{padding:"6px 14px",borderRadius:100,border:"none",cursor:"pointer",fontSize:10,fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",transition:"all 0.2s",
                      background:selFecha===n?"linear-gradient(135deg,#fbbf24,#f97316)":"rgba(255,255,255,0.07)",
                      color:selFecha===n?"white":"rgba(255,255,255,0.45)",
                      boxShadow:selFecha===n?"0 2px 12px rgba(251,191,36,0.4)":"none"}}>
                    F{n}
                  </button>
                ))}
              </div>
            ...
            {selFecha!==null&&(
              <div style={{padding:"12px 12px 0"}}>
                ...
                {fechaError&&<ErrMsg msg={fechaError} onRetry={()=>{ setFechaCache(p=>{const n={...p};delete n[selFecha];return n;}); loadFecha(selFecha); }}/>}
                {!fechaLoading&&!fechaError&&fechaCache[selFecha]&&(()=>{
                  ...
                  return (
                    <div>
                      ...
                      {jug.length>0&&(
                        <>
                          <div style={{fontSize:9,fontWeight:700,letterSpacing:2,color:"rgba(255,255,255,0.3)",textTransform:"uppercase",marginBottom:8,paddingLeft:4}}>
                          ...
                          {jug.map((p,idx)=>{
                            const globalP = jugadoras.find(j=>j.nombre?.toLowerCase()===p.nombre?.toLowerCase());
                            const gRank   = globalP ? jugadoras.indexOf(globalP) : idx;
                            const isMe    = p.nombre?.toLowerCase()===myName?.toLowerCase();
                            const medal   = rankMedal(idx);
                            return (
                              <div key={p.nombre}
                                style={{display:"grid",gridTemplateColumns:"30px 1fr 42px 42px 50px",gap:4,alignItems:"center",padding:"7px 10px",margin:"0 0 3px",borderRadius:10,
                                ...
                                <div style={{textAlign:"center",fontSize:idx<3?14:10,fontWeight:900,color:idx===0?"#fbbf24":idx===1?"#e2e8f0":idx===2?"#fb923c":"rgba(255,255,255,0.28)"}}>{medal||(idx+1)}</div>
                                <div style={{display:"flex",alignItems:"center",gap:6,minWidth:0}}>
                                  <Avatar name={p.nombre} rank={gRank} size={24}/>
                                ...
                                <div style={{textAlign:"center",fontSize:12,fontWeight:900,color:diffColor(p.diff)}}>
                                  {p.diff>0?"+":""}{p.diff}
                                </div>
                              </div>
                            );
        ...
        {tab==="insc"&&(
          <div style={{padding:"14px 12px"}}>
            <div style={{background:"linear-gradient(135deg,rgba(232,79,160,0.18),rgba(176,111,196,0.1))",border:"1px solid rgba(232,79,160,0.35)",borderRadius:18,padding:"20px",marginBottom:14,textAlign:"center"}}>
              ...
              <div style={{fontSize:15,fontWeight:900,color:"white",marginBottom:4}}>Próxima Fecha — Fecha {fechaActual+1}</div>
        ...
        {tab==="perfil"&&(
          myPlayer
            ...
            : <div style={{textAlign:"center",color:"rgba(255,255,255,0.3)",padding:"60px 24px",fontSize:13}}>
                ...
                Tu nombre (<strong style={{color:"white"}}>{myName}</strong>) no está en la tabla aún.<br/>
